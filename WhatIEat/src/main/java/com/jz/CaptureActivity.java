...
import com.jz.api.ApiAnalysis;
import com.jz.camera.CameraManager;
import com.jz.camera.ShutterButton;
import com.jz.language.Ingredient;
import com.jz.language.LanguageCodeHelper;
import com.jz.language.TranslateAsyncTask;
...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback,
     ...
     */
    static final String DOWNLOAD_BASE = "http://tesseract-com.jz.googlecode.com/files/";

    /**
     * Download filename for orientation and script detection (OSD) data.
     */
    static final String OSD_FILENAME = "tesseract-com.jz-3.01.osd.tar";
    ...
    public void onCreate(Bundle icicle) {
        ...
        // All the camera related methods are in this class
    ...
    void onShutterButtonPressContinuous() {
        ...
        if (lastResult != null) {
            ...
            Toast toast = Toast.makeText(this, "OCR failed on Button Click. Please try again.", Toast.LENGTH_SHORT);
    ...
    private File getStorageDirectory() {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            ...
            //          // "/mnt/sdcard/Android/data/com.jz/files/"
    ...
    boolean handleOcrDecode(OcrResult ocrResult) {
        ...
        // Stop camera
        //SurfaceView surfaceView2 = (SurfaceView) findViewById(R.id.preview_view);
        //surfaceView2.setBackgroundColor(0Xffffffff);
        ...
        // Prepare JSON message - result of the OCR operation
        JSONObject jsonPost = new JSONObject();
        try {
            jsonPost.put("Text", ocrResult.getText()); // Preparing JSON message, key must be "Text"
            //jsonPost.put("Text", "sugar, salt, sfsldfjlsvlskdv"); // For testing
        ...
        final List<Ingredient> apiResponse = ApiAnalyser.Post(jsonPost); // makes HTTP POST request (final means it cannot be changed)
        ...
        for (Ingredient ingredient : apiResponse) {
            ...
            Log.i("CaptureActivity - API", ingredient.IngredientName + " " +
                                            ingredient.IngredientDescription + " " +
                                            ingredient.IngredientDangerLevel + " " +
                                            ingredient.FuzzyDistance);
        }
        //String[] ingredientNames = {"Sugar","Salt","Whole"};
        //String firstIngredient = apiResponse.get(0).IngredientName; // For the test - just gets the first ingredient from the list

        // Display a list of ingredients on the screen
        ArrayAdapter adapter = new ArrayAdapter<String>(this, R.layout.activity_listview, ingredientNames);
        final ListView ingredientsListView = (ListView) findViewById(R.id.ingredients_list);
        ingredientsListView.setAdapter(adapter);

        // On List Item Click event declaration
        // Tapping the Item in the ingredients list will open a new Activity window (DetailsActivity) and show more details
        ingredientsListView.setOnItemClickListener(
            new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    String selectedIngredientName = (String) parent.getItemAtPosition(position);

                    // Get the details of the ingredient by it's name
                    Ingredient selectedIngredient = null;
                    for(Ingredient i : apiResponse)
                    {
                        if (i.getName().equals(selectedIngredientName))
                        {
                            selectedIngredient = i;
                            Log.i("CaptureActivity - API", i.IngredientName + " " + i.IngredientDescription);
                            break;
                        }
                    }

                    // Pass the details of the ingredient to the DetailsActivity screen
                    Intent intent = new Intent().setClass(CaptureActivity.this, DetailsActivity.class);
                    if (selectedIngredient != null)
                    {
                        intent.putExtra("MY_KEY", selectedIngredient.IngredientDescription + "!!!");
                    }
                    else
                    {
                        intent.putExtra("MY_KEY", "We do not have the description for this ingredient. Object is NULL");
                    }
        ...
        //int scaledSize = Math.max(22, 32 - ocrResult.getText().length() / 4); // bigger font for shorter text
        //ocrResultTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP, scaledSize);

        // Display the language of the OCR (under the image)
